...
### 6.3 Multi-join reads (proposed design)

Multi-join is a separate follow-up after the binary API is stable, and needs a
new AST: the current `JoinSelect<L, R>` stores exactly one `JoinClause<R>`.
Modelling N joins as `JoinSelect3`, `JoinSelect4`, … does not scale — it forces
a combinatorial explosion of typed tuples and renderers. The proposed shape
instead generalises the binary join into an ordered chain of typed *edges*,
plus a row-view addressed by `Table<E>` rather than by tuple position.

#### 6.3.1 `on` infers the joined table from its right-hand column

```kotlin
/** Pairs a left and right column for a join condition. The right column's
 *  owning table (`Column.table`) is the table that gets joined — never inferred
 *  from foreign keys, always written explicitly by the caller. */
data class JoinPair<V>(val left: Column<*, V>, val right: Column<*, V>)

infix fun <V> Column<*, V>.on(other: Column<*, V>): JoinPair<V> = JoinPair(this, other)

/** One step in a join chain: `leftColumn = rightColumn`, with an explicit [JoinType]. */
data class JoinStep<V>(
    val leftColumn: Column<*, V>,
    val rightColumn: Column<*, V>,
    val type: JoinType,
) {
    val rightTable: Table<*> get() = rightColumn.table
}
```

`JoinType` gains `INNER` and `RIGHT` alongside the existing `LEFT`, since a
join chain may mix kinds.

#### 6.3.2 Two equivalent entry points: block DSL and vararg DSL

Block form — `join`/`left`/`right` inside the scope choose the join kind per step:

```kotlin
val q = join(Orders) {
    left(Orders.customerId on Customers.id)
    join(Customers.sectionId on Sections.id)   // INNER JOIN by default keyword inside the scope
}.where { Orders.status eq "PAID" }
```

Vararg form — top-level `inner`/`left`/`right` wrap a `JoinPair` into a typed
`JoinStep`, so a join chain reads as a flat list of typed edges:

```kotlin
val q = join(
    Orders,
    left(Orders.customerId on Customers.id),
    inner(Customers.sectionId on Sections.id),
)
```

Both build the same immutable AST:

```kotlin
data class MultiJoinSelect(
    val rootTable: Table<*>,
    val steps: List<JoinStep<*>>,
    ...
    init {
        require(steps.isNotEmpty()) { "multi-join requires at least one step" }
        val tables = listOf(rootTable) + steps.map { it.rightTable }
        require(tables.distinct().size == tables.size) {
            "joining the same table twice requires an alias model (open question #5)"
        }
    }
}
```

Same-table-twice remains blocked until an alias model exists (open question #5
is unchanged by this design).

#### 6.3.3 Row materialisation: a row-view addressed by `Table<E>`

Instead of a positional tuple (`JoinedRow3.a/b/c`), the result row is a view
that callers index by table descriptor — this scales to N tables without new
generated types per arity:

```kotlin
class MultiJoinedRow internal constructor(
    private val row: Row,
    private val offsets: Map<Table<*>, Int>,
) {
    operator fun <E> get(table: Table<E>): E? =
        if (sideIsNull(table, row, offsets.getValue(table))) null
        else table.fromRow(PositionalRow(row, nameToIndex(table.columns, offsets.getValue(table))))
}
```

`offsets.getValue` throws a clear error if a caller indexes a table that is not
part of the query — the same fail-fast posture as `ProjectedRow`/`AggRow`
indexing by column/expression key.

#### 6.3.4 Execution

`renderMultiJoinSelect` walks `steps` in declaration order, emitting
`<rootTable> <step1.type.sql> <step1.rightTable> ON <leftCol> = <rightCol> <step2...>`,
reusing `renderQualifiedColumn`, `PredicateRenderer`, and `appendPagination`.
`Session` gains `fetchAllJoined`/`streamJoined` over `MultiJoinSelect` returning
`MultiJoinedRow`, plus the same `fetchAll`/`fetchOne`/`stream` overloads used
for binary joins (§6.1), but with `map: MultiJoinedRow.() -> A`:

```kotlin
val orders: List<OrderFull> = aggo.read {
    fetchAll(
        join(Orders) {
            left(Orders.customerId on Customers.id)
            join(Customers.sectionId on Sections.id)
        },
    ) {
        OrderFull(order = this[Orders]!!, customer = this[Customers], section = this[Sections])
    }
}
```

#### 6.3.5 KSP generation for multi-join projections

Hand-writing `this[Table]!!` glue for every DTO field repeats the same mapping
boilerplate the KSP DTO model in §6.2 already targets for binary joins. The
multi-join design extends naturally: KSP generates the `row -> DTO` mapper —
**not** the join descriptor itself, which stays an explicit, type-checked
`join { }` / vararg value the developer writes (per goal #8 — every join edge
must be explicit, never inferred from FK metadata).
...
Generated code at compile time (no runtime reflection, no `KClass`):

```kotlin
internal fun mapOrderFullDto(row: MultiJoinedRow): OrderFullDto =
    OrderFullDto(
        order = row[OrdersTable]!!,
        customer = row[CustomersTable]?.let { CustomerDto(id = it.id, name = it.name) },
        section = row[SectionsTable]?.let { SectionDto(id = it.id, label = it.label) },
    )
```

The generator only needs the DTO's declared shape (field names, nullability,
nested DTO constructors) plus the `Table<E>` each branch maps from — it does
not need to discover the join graph; the `MultiJoinSelect` value passed to
`fetchAll`/`fetchOne` is supplied by the caller and type-checked independently.
This keeps KSP's job purely mechanical (row access → constructor call), exactly
matching the zero-reflection / no-hidden-mapping invariants in §4 and §9.
